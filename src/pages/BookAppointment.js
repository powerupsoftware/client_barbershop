...
const BookAppointment = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(
        "Ocurrió un error al agendar tu cita. Por favor intenta de nuevo."
      );
    } finally {
  ...
  return (
    <Container>
      ...
      {client && (
        <Welcome>
          <WelcomeText>
            ¡Hola <span>{client.name}</span>! ¿Qué servicios te gustaría agendar
            hoy?
          </WelcomeText>
        </Welcome>
      )}
      ...
      <Button
        ...
        disabled={
          isSubmitting || selectedServices.length === 0 || !selectedTime
        }
      >
        {isSubmitting ? (
          <Loader text={null} />
